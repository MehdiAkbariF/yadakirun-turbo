...
import { ContactForm } from '@monorepo/design-system/src/components/organisms/ContactForm/ContactForm';
...
import { ContactUsPageData } from '@monorepo/api-client/src/types/contact.types';

// ✅ نقشه در کامپوننت کلاینت به صورت داینامیک ایمپورت می‌شود
const MapSelector = dynamic(
  () => import('@monorepo/design-system/src/components/organisms/MapSelector/MapSelector').then(mod => mod.MapSelector),
  { ssr: false, loading: () => <div className="h-[350px] bg-gray-100 rounded-2xl animate-pulse" /> }
);

interface ContactUsClientProps {
  pageData: ContactUsPageData | null;
}
...
export function ContactUsClient({ pageData }: ContactUsClientProps) {
  ...
  const contactInfo = {
    phone: "021-33988013",
    mobile: "0935 7744 749",
    email: "info@yadakirun.com",
    postalCode: "1143933534",
    address: "تهران، خیابان جمهوری، خیابان ملت، کوچه کاوه، کوچه افسانه، پلاک 7، واحد 10"
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-11 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <ContactItem icon={<Phone />} title="تلفن ثابت" value={contactInfo.phone} href={`tel:${contactInfo.phone}`} />
              <ContactItem icon={<Smartphone />} title="موبایل" value={contactInfo.mobile} href={`tel:${contactInfo.mobile}`} />
              <ContactItem icon={<Mail />} title="ایمیل سازمانی" value={contactInfo.email} href={`mailto:${contactInfo.email}`} />
              <ContactItem icon={<MapPin />} title="کد پستی" value={contactInfo.postalCode} />
            </div>

            <ContactItem icon={<MapPin />} title="نشانی دفتر مرکزی" value={contactInfo.address} isAddress />
          ...
          <div className="lg:col-span-3 lg:sticky lg:top-32">
            <div className="bg-bg-surface border border-border-secondary rounded-2xl p-6 shadow-lg">
              ...
              <Label color="secondary" className="leading-loose mb-6 text-justify" size='sm'>
                لطفا پیش از تماس با ما، پرسش‌های متداول را بخوانید...
              </Label>
              <Button href="/faq" variant="primary" fullWidth size="md" rightIcon={<ArrowLeft size={18} />} className='mt-5'>
