...
import { CheckCircle, Car, Wrench, ShieldCheck, Headset, X, ChevronLeft } from "lucide-react";
...
const productData = {
  ...
  description: {
     sections: [
        { title: "دستیگره بیرونی درب لندرور و پاژن سانتانا چیست؟", content: "دستیگره بیرونی درب به کاربر اجازه می‌دهد تا به‌راحتی درب را باز یا بسته کند. این قطعه با طراحی ارگونومیک و مواد مقاوم، به‌گونه‌ای ساخته شده که دوام و عملکرد مطلوبی را حتی در شرایط سخت جاده‌ای تضمین کند." },
        { title: "کاربرد محصول", content: "کاربرد اصلی دستیگره بیرونی درب، فراهم کردن دسترسی آسان و مطمئن به داخل خودرو است." }
     ],
     specs: [
        { title: "جنس", content: "ساخته شده از مواد مقاوم پلاستیکی یا فلزی با روکش ضدخوردگی." },
        { title: "طراحی ارگونومیک", content: "به‌گونه‌ای طراحی شده که استفاده آسان و راحت را برای کاربر فراهم کند." },
     ]
  },
  performance: "عالی"
};
...
const serviceItems = [
  { icon: <Wrench size={28}/>, title: "مهلت مرجوعی", description: "تا ۲۴ ساعت پس از تحویل در صورت مغایرت" },
  { icon: <ShieldCheck size={28}/>, title: "ضمانت اصالت", description: "تضمین اصل بودن تمام قطعات" },
  { icon: <Headset size={28}/>, title: "پشتیبانی", description: "مشاوره تخصصی رایگان قبل از خرید" },
];
           ...
           <div className="order-1 lg:col-span-7 lg:row-start-1">
              <div className="flex flex-col lg:flex-row p-4 bg-surface rounded-3xl shadow-sm border border-border-secondary">
                 ...
                 <div className="lg:w-1/3 mb-6 lg:mb-0">
                    <ProductGallery images={productData.images} alt={productData.name} />
                 </div>

                 {/* Product Summary */}
                 <div className="flex flex-col w-full lg:w-2/3 p-4 sm:p-6 lg:p-8">
                    <Label as="h1" size="xl" weight="extra-bold" className="text-heading mb-2">
                       {productData.name}
                    </Label>
                    <Label size="sm" color="placeholder" className="font-sans mb-6 block">
                       ...
                       <>
                         ...
                         <div className="space-y-4">
                            <div className="flex items-baseline gap-4">
                               <Label weight="bold" size="base">بسته بندی:</Label>
                               <Label size="sm" color="secondary">
                                  {selectedPackage ? selectedPackage.label : "یک مورد را انتخاب کنید"}
                               </Label>
                            </div>
                            <div className="flex flex-wrap gap-3">
                               {productData.packagingOptions.map((option) => (
                                  <button
                                     ...
                                     className={`px-4 py-2 rounded-lg border-2 transition-all text-xs md:text-sm font-semibold 
                    ...
                    <div className="flex flex-wrap justify-between items-center gap-4 w-full mb-6">
                       ...
                       {perfStyle && (
                          <div className="flex items-center gap-2">
                             <Label size="xs" color="secondary" className="opacity-80">عملکرد:</Label>
                    ...
                    <div className="bg-bg-secondary border border-border-secondary rounded-lg p-4 mb-6">
                       <div className="flex items-center gap-3 mb-4">
                          ...
                          <Label weight="bold" size="base">ویژگی‌ها</Label>
                       </div>
                       <div className="space-y-3">
                          <div className="flex justify-between">
                             <Label size="sm" color="secondary">برند:</Label>
                             <Label size="sm" weight="bold">{productData.brand}</Label>
                          </div>
                          <div className="flex justify-between">
                             <Label size="sm" color="secondary">خودروساز:</Label>
                             <Label size="sm" weight="bold">{productData.carMaker}</Label>
                          </div>
                       </div>
                    </div>

                    {/* Suitable For */}
                    <div className="bg-bg-secondary/50 px-4 py-4 rounded-lg">
                       <div className="flex items-center justify-between gap-4 mb-4">
                          ...
                          <button className="text-xs font-bold text-brand-primary flex items-center gap-1 hover:text-brand-primary-hover transition-colors">
                             مشاهده بیشتر <ChevronLeft size={16} />
                          </button>
                       </div>
                       ...
                       <div className="flex flex-wrap gap-2">
                          {productData.suitableFor.map((car, i) => (
                             <Badge key={i} variant="secondary" className="px-3 py-1 font-medium border border-border-secondary">
                                {car}
                             </Badge>
                          ))}
                       </div>
        ...
        >
           <div className="space-y-8 mt-6">
              {productData.description.sections.map((section, idx) => (
                 <div key={idx}>
                    <Label as="h3" size="lg" weight="bold" className="mb-2 block">{section.title}</Label>
           ...
           <Label as="h3" size="lg" weight="bold" className="mb-4 block">مشخصات</Label>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              {productData.description.specs.map((spec, idx) => (
                 <div key={idx} className="flex flex-col sm:flex-row sm:justify-between border-b border-border-secondary pb-2 last:border-0">
                    <Label size="sm" color="placeholder" weight="bold" className="mb-1 sm:mb-0">{spec.title}</Label>
                    <Label size="sm" weight="medium" color="primary">{spec.content}</Label>
                 </div>
              ))}
           </div>
        </ContentSection>
